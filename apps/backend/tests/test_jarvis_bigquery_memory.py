"""
Comprehensive tests for api/jarvis_bigquery_memory.py

Tests the BigQuery-inspired memory system including:
- Enums and dataclasses
- Database setup and schema
- Command storage and retrieval
- Embedding generation with unified embedder
...
from api.jarvis_bigquery_memory import (
    MemoryType,
    MemoryTemplate,
    SemanticMemory,
...
@pytest.fixture
def memory(temp_db):
    """Create a JarvisBigQueryMemory instance with temp database"""
    mem = JarvisBigQueryMemory(db_path=temp_db)
    yield mem
    mem.conn.close()
...
# ============================================================================
# Test JarvisBigQueryMemory Initialization
# ============================================================================

class TestJarvisBigQueryMemoryInit:
    """Tests for JarvisBigQueryMemory initialization"""

    def test_init_creates_database(self, temp_db):
        """Initialization creates database file"""
        os.unlink(temp_db)

        memory = JarvisBigQueryMemory(db_path=temp_db)
    ...
    def test_init_creates_parent_dirs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            memory = JarvisBigQueryMemory(db_path=db_path)
    ...
    def test_init_sets_performance_pragmas(self, memory):
        """Performance PRAGMAs are set"""
        cursor = memory.conn.execute("PRAGMA cache_size")
        cache_size = cursor.fetchone()[0]
        assert cache_size == -64000  # 64MB
...
class TestUpdatePatternStats:
    ...
    def test_updates_existing_pattern(self, memory):
        ...
        assert row['avg_execution_time'] == 3.0
