...
class WorkflowOrchestrator:
    ...
    def _transition_to_stage(
        ...
        if next_stage.stage_type == StageType.AUTOMATION and next_stage.automation:
            ...
            self._execute_automation_stage(work_item, next_stage, user_id)
    ...
    def _execute_automation_stage(
        ...
        if automation_type == "n8n":
            ...
            self._trigger_n8n_automation(work_item, stage, user_id)

        elif automation_type == "local_ai":
            # Run local AI processing
            self._run_local_ai_automation(work_item, stage, user_id)

        elif automation_type == "custom":
            # Run custom script
            self._run_custom_automation(work_item, stage, user_id)

        else:
            logger.warning(f"Unknown automation type: {automation_type}")

    def _trigger_n8n_automation(
        self,
        work_item: WorkItem,
        stage: Stage,
        user_id: str
