  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-auto p-2 font-mono text-xs">
        {logs.map((log) => (
          <div key={log.id} className="py-1 px-2 hover:bg-gray-50 dark:hover:bg-gray-900 rounded">
            <div className="flex items-start mb-1">
              <span className="text-gray-500 dark:text-gray-500 mr-2 flex-shrink-0">{log.timestamp}</span>
              <span className={`uppercase mr-2 flex-shrink-0 ${getLevelColor(log.level)}`}>
                [{log.level}]
              </span>
            </div>
            <div className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-words ml-2 pl-2 border-l-2 border-gray-300 dark:border-gray-700">
              {log.message}
            </div>
          </div>
        ))}
