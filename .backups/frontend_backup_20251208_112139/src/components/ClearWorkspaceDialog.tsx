...
export function ClearWorkspaceDialog() {
  ...
  useEffect(() => {
    const openHandler = (event: CustomEvent) => {
      const shouldKeepLogs = event.detail?.keepLogs ?? false
      setKeepLogs(shouldKeepLogs)
      setOpen(true)
    }
    window.addEventListener('open-clear-workspace', openHandler as EventListener)
    return () => window.removeEventListener('open-clear-workspace', openHandler as EventListener)
  }, [])
  ...
  const handleClearWorkspace = async () => {
    ...
    try {
      ...
      // Only clear logs if requested (rail trash icon, not file X button)
      if (!keepLogs) {
        clearLogs()
      }
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 ml-9 bg-blue-50 dark:bg-blue-950/30 border-l-2 border-blue-400 dark:border-blue-600 pl-3 py-2">
          {keepLogs
            ? '💡 Your logs and saved queries will be preserved.'
            : '💡 Your saved queries in the Library will be preserved.'}
        </p>
