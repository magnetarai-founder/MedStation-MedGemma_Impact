#!/bin/bash
# Setup script to create 'omni' command globally
...
if grep -q "alias omni=" "$SHELL_RC" 2>/dev/null; then
    ...
    echo "" >> "$SHELL_RC"
    echo "# OmniStudio alias" >> "$SHELL_RC"
    echo "$ALIAS_LINE" >> "$SHELL_RC"
fi
...
echo "  3. Run: omni"
echo ""
echo "The command will start OmniStudio from anywhere!"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    source "$SHELL_RC"
    echo "✓ Done! You can now run 'omni' from anywhere"
else
